...
namespace Sndpbag\AdminPanel\Commands;
...
class InstallCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $userModelClass = config('admin-panel.user_model');
        ...
        if (!class_exists($userModelClass)) {
            ...
            $myTrait = \Sndpbag\AdminPanel\Traits\HasRolesAndPermissions::class; // সম্পূর্ণ পাথ
            ...
            if (!in_array($myTrait, $traits)) {
                ...
                $this->info("Please add the 'use Sndpbag\AdminPanel\Traits\HasRolesAndPermissions;' trait to your {$userModelClass} file first.");
                ...
                if ($this->confirm('Do you want to create or assign a Super Admin user now?', true)) {
                    $superAdminRoleSlug = config('admin-panel.super_admin_role', 'super-admin');
    ...
    protected function createDefaultRoles()
    {
        $roles = [
            [
                'name' => 'Super Admin',
                'slug' => config('admin-panel.super_admin_role', 'super-admin'),
